# Nufe

[English](README.md) | 中文

**Nufe** 是一门正在建设中的人工语言。[点此进入网站](https://nufe-language.vercel.app)。

它不追求模仿某一种自然语言，而是尝试建立一种更清楚、更稳定、更容易被人和 AI 共同理解的表达系统。Nufe 的目标不是把语言变成机器代码，而是让词、句子和意义之间的关系更容易被说明、学习、整理和扩展。

你可以把 Nufe 想象成一门“概念结构很清楚”的语言：它可以表达日常动作，也很适合表达存在、关系、世界、意识、行动、判断、因果和抽象概念。

## 一眼理解 Nufe

Nufe 有几个很鲜明的设计方向：

- **音节有限**：所有词都由规则音节组成，读写边界清楚。
- **词类清楚**：词按功能分为实体、动作、性质、功能四类。
- **语序稳定**：基本句子按“主语 + 谓语 + 宾语”组织。
- **修饰前置**：修饰语放在被修饰词前面，直接修饰后面紧邻的词。
- **语法显性**：时态、体态、否定、连接等尽量由明确的功能词表达。
- **方便扩展**：外来词、专名、数词和抽象词都可以按规则继续扩展。
- **中古汉语高保真支持**：中古汉语读音可以系统地同化为合法的 Nufe 音形，并尽量保留历史音系中的关键区别。

## 声音和书写

Nufe 使用拉丁字母书写，词与词之间用空格分隔。

它的基本音节结构是：

```text
(C)(G)V(N)
```

也就是：

```text
辅音 + 介音 + 元音 + 鼻音
```

其中元音必须出现，其他部分可以省略。比如：`mi`、`ti`、`pa`、`lu`、`muo`、`liew`、`pale`。

Nufe 的声音系统刻意保持简洁：5 个基本元音、8 个双元音、一组常用辅音，以及统一的鼻音形式。这样做的好处是，词形容易控制，也方便之后稳定地引入外来词。

### 发音简介

Nufe 的元音系统包括 5 个单元音、8 个写作“介音 + 元音”的双元音，以及带鼻音尾 `-w` 的形式。鼻音尾在实际发音中可读作 [ŋ]、[n] 或 [m]。

| 类型 | 形式与发音 |
| --- | --- |
| 单元音 | `a` [a]，`i` [i]，`u` [u]，`e` [ɛ]，`o` [o] |
| 双元音 | `ia` [ja]，`ie` [jɛ]，`io` [jo]，`iu` [ju]，`ua` [wa]，`ue` [wɛ]，`uo` [wo]，`ui` [wi] |
| 单元音鼻音 | `aw` [aŋ/an/am]，`iw` [iŋ/in/im]，`uw` [uŋ/un/um]，`ew` [ɛŋ/ɛn/ɛm]，`ow` [oŋ/on/om] |
| 双元音鼻音 | `iaw` [jaŋ/jan/jam]，`iew` [jɛŋ/jɛn/jɛm]，`iow` [joŋ/jon/jom]，`iuw` [juŋ/jun/jum]，`uaw` [waŋ/wan/wam]，`uew` [wɛŋ/wɛn/wɛm]，`uow` [woŋ/won/wom]，`uiw` [wiŋ/win/wim] |

Nufe 使用 18 个基本辅音：

| 类型 | 字母与发音 |
| --- | --- |
| 唇音 | `p` [pʰ]，`b` [b]，`m` [m]，`f` [f]，`v` [v] |
| 舌音 | `t` [tʰ]，`d` [d]，`n` [n]，`s` [s]，`z` [z] |
| 软腭音与喉音 | `k` [kʰ]，`g` [g]，`q` [ŋ]，`h` [h]，`r` [ɦ] |
| 塞擦音 | `c` [tʃʰ]，`j` [dʒ] |
| 边音 | `l` [l] |

如果一个音节没有开头辅音，完整写法会在前面加 `y`，如 `ya`、`yi`、`yu`、`ye`、`yo`。在词首位置，这个 `y` 可以省略，所以 `ya` 可以写成 `a`。

词内读音也有固定规则：最末音节读低降调；倒数第二个音节读高平调，是整个词最明亮、最容易辨认的部分；更前面的音节读低平调。书写时，词与词之间用空格分隔。

## 中古汉语支持

Nufe 包含一套系统的中古汉语同化模型。它把中古汉语音类映射到合法的 Nufe 音节中，并在 Nufe 声音系统允许的范围内尽量保留有历史意义的区别。

在当前按《广韵》小韵进行的统计中，Nufe 主体同化方案能在 `3874` 个小韵中保留 `1160` 个不同表面形式，形式区分率为 `29.94%`。用同一口径统计，普通话不带调只能保留 `406` 个形式，形式区分率为 `10.48%`。也就是说，Nufe 对中古汉语的支持具有很高的保真度，其区分能力超过无声调普通话的两倍以上，同时仍然保持在 Nufe 规则书写和规则发音之内。

## 四类词

Nufe 最核心的设计，是把词按句法功能分成四类。

| 词类 | 作用 | 例子 |
| --- | --- | --- |
| 实体 | 指人、物、地点、概念 | `mi` 我，`ti` 你，`pa` 东西，`gia` 人 |
| 动作 | 作谓语，表示行为或状态 | `pale` 存在，`muo` 吃，`toli` 说，`da` 在 |
| 性质 | 修饰实体、动作或性质 | `si` 小的，`ba` 大的，`liew` 明亮的 |
| 功能 | 表达语法关系 | `na` 不，`aw` 现在，`iw` 过去，`uw` 将来 |

这意味着，学习 Nufe 时不用先面对大量复杂变形。你只需要先判断一个词在句子里做什么：它是在指称一个东西、表示一个动作、描述一个性质，还是承担语法功能。

## 最基本的造句方法

Nufe 的核心语序是：

```text
主语 + 谓语 + 宾语
```

例如：

| Nufe | 中文 |
| --- | --- |
| `mi pale.` | 我存在。 |
| `mi muo.` | 我吃。 |
| `mi mela ti.` | 我爱你。 |
| `mi da mife.` | 我在城市。 |
| `mi qe ti.` | 我知道你。 |

如果动作不需要对象，就可以只有“主语 + 谓语”：

```text
mi pale.
我存在。
```

如果动作需要对象，就把对象放在动作后面：

```text
mi mela ti.
我爱你。
```

## 修饰规则

Nufe 的修饰语放在前面，并且直接修饰后面紧邻的词。

```text
liew ka
明亮的火
```

```text
ve gia
许多人
```

```text
mi dili
我的家 / 我的归属之处
```

多个同类成分可以用 `e` 连接：

```text
mi e ti pale.
我和你存在。
```

## 时间和状态

Nufe 不通过动词变形表达时态，而是在动作词前放功能词。

| 功能词 | 含义 |
| --- | --- |
| `aw` | 现在 |
| `iw` | 过去 |
| `uw` | 将来 |
| `ew` | 完成 |
| `ow` | 进行 |

例如：

```text
mi iw muo.
我过去吃。
```

```text
mi ow muo.
我正在吃。
```

时态和体态可以组合成更紧凑的形式：

| 形式 | 含义 |
| --- | --- |
| `iaw` | 现在完成 |
| `uaw` | 现在进行 |
| `iew` | 过去完成 |
| `iow` | 过去进行 |
| `uew` | 将来完成 |
| `uow` | 将来进行 |

例如：

```text
mi uaw muo.
我现在正在吃。
```

## 判断、否定和句子包裹

Nufe 用动作词 `ne` 表示“是 / 被判断为”：

```text
mi ne gia.
我是人。
```

否定可以用功能词 `na`：

```text
mi na muo.
我不吃。
```

当一个完整句子需要变成“一个东西”来被谈论时，可以用：

```text
i [句子] u
```

例如：

```text
mi qe i ti pale u.
我知道你存在。
```

这里 `ti pale` 是“你存在”，外面加上 `i ... u` 后，整个句子就能作为 `qe`“知道”的对象。

## 数字也按规则生成

Nufe 的基本数根是：

| 数值 | Nufe |
| --- | --- |
| 1 | `pa` |
| 2 | `fa` |
| 3 | `ma` |
| 4 | `ta` |
| 5 | `sa` |
| 6 | `na` |
| 7 | `ka` |
| 8 | `ha` |
| 9 | `qa` |
| 0 | `la` |

数根可以直接拼合成数元。例如：

```text
pafama
123
```

在数元后加 `ce`，表示“多少个”；加 `co`，表示“第几个”：

```text
pace
一个的
```

```text
paco
第一个的
```

## 试着读几句

| Nufe | 中文 |
| --- | --- |
| `mi lew bu mi dili.` | 我想回家。 |
| `lawpa da zifo.` | 灯在家里。 |
| `lu da dow iwfa.` | 水在建筑下面。 |
| `ti so i mi via u.` | 你使我活着。 |

Nufe 仍然在生长中。它已经有了清晰的骨架，也逐渐显露出自己的气质：简洁、可扩展、重视概念结构，并认真考虑未来人与 AI 共同使用语言时，语言本身应该如何被设计。
